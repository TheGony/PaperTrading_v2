...
[종목 선정 방식]
■ ORB 풀 (09:01 1회 고정, 갱신 없음)
  거래대금 상위 50 + 거래량급증 50 병합
  → 갭 +2~+7% / 현재가≥시가 / 윗꼬리≤5% / 음봉 1개 이하
  → 스코어: 거래량증가 40% + 등락률 35% + 거래대금 25%
  → 상위 15종목 선정 / ETF·ETN·스팩 제외

■ 모멘텀 풀 (ORB와 완전 분리)
  장 초반 (09:00~09:45): 거래대금 상위 Top30 → 거래대금+등락률 스코어링 / 2분 갱신
  장 중반 (09:45~14:40): 거래량급증 Top50 → 거래량급증률+등락률+RSI+외인기관 / 10분 갱신
  장 후반 (14:40~15:30): 기존 선정 종목 유지 (신규 선정 없음)

[매매 전략]
■ ORB 전략 (09:05~09:25, orb_candidates 전용)
  진입: 09:00~09:04 고가 최대값(ORB 고점) 돌파 + 거래량 > 직전봉×1.5 + RSI 55~68
        추격방지: 돌파 기준 +1% 초과 시 진입 금지 / 최대 2회 진입
  손절: max(ORB 저점, -2%) 중 타이트한 기준
  트레일링 (4단계):
    peak < 2%  → gap 1.2% / peak < 4%  → gap 1.5%
    peak < 7%  → gap 2.0% / peak ≥ 7%  → gap 2.5%
  수익반납방지: peak ≥ 1.5% 도달 후 수익률 0% 미만 시 즉시 매도

■ 모멘텀 전략
  공통 청산: 데드크로스(MA{chart_short} < MA{chart_long}) AND RSI < 45
  공통 손절: 수익률 ≤ slr 즉시 매도
  조기 손절: 진입 3분 이내 수익률 < -1.2% 시 즉시 매도
  트레일링 (4단계):
    peak ≥ 7%  → gap 1.5% / peak ≥ 4%  → gap 2.0%
    peak ≥ 2%  → gap 2.5% / peak < 2%   → phase 기본값

  [장 초반] 직전 3봉 고점 돌파 + 거래량 > 직전봉 + 현재가 > 시가 + RSI 50~68
            최대 5회 진입 / 손절 -2% 고정 / 트레일링 2% 고정

  [장 중반] 직전 5봉 고점 돌파 + 거래량 > 직전봉 + (RSI > 55 OR 현재가 > MA{chart_long})
            손절 {slr}% / 트레일링 {tsg}% (동적 4단계 적용)

  [장 후반] 직전 5봉 고점 돌파 + 거래량 > 직전봉 + RSI > 60 AND 현재가 > MA{chart_long}
            손절 {slr}% / 트레일링 2.5% 고정

[안전장치]
• 추격매수 방지: 돌파 기준점 대비 +1% 초과 시 진입 금지
• 쿨다운: 매도 후 20분간 동일 종목 재매수 금지
• 당일 손실 2회 이상 종목 당일 거래 금지
• 1회 매수 금액: 총자산 × {brt}%"""
